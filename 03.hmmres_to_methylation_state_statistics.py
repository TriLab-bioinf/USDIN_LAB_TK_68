...
filename = "nhmmer_q0.res.40_refs.txt"
...
df_join = df1.set_index('ID').join(df2.set_index('ID'))
...
for i in range(df_join.shape[0]):
    ...
    df_join['Phred'].iloc[i] = float(re.sub('^.*_q', '', df_join.index[i]))
    ...
    #df_join['numT'].iloc[i] = list(df_join["sequence"].iloc[i]).count('T')
...
sns.boxplot(x='class', y='score', data=df_join, order=["<20", "20~25", "25~30","30~35", "35~40", ">40"]).set(title='Phred score vs alignment score')
plt.show()
...
df_join.to_csv('nhmmer_q0.res.40_refs.csv')
...
alignDict2 = {}
alignDict2 = {k: v for k, v in alignDict.items() if k in df_filter.index}
...
outfile = open("Methylation_statistics_nhmmer_q30_dash1.txt", 'w')
